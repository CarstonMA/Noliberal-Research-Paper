...
# Output: merged_fraser_heritage.csv

library(tidyverse)
if (!requireNamespace("countrycode", quietly = TRUE)) install.packages("countrycode")

# 1. Load data
fraser <- read_csv("Fraser Time Series.csv", show_col_types = FALSE)
...
merged %>%
  ...
  arrange(ISO3, Year) %>%
  write_csv("merged_fraser_heritage.csv", na = "")

message("Saved merged_fraser_heritage.csv: ", nrow(merged), " rows, ", ncol(merged), " cols")
